"""
A simple sorting algorithm visualizer created by Kevin Lai utilizing Tkinter.

This tool is for demonstration purposes only and should not be redistributed
for commercial or educational use. If redistributed, please credit Kevin Lai
 - GitHub: https://github.com/Silk8Worm
 - LinkedIn: https://www.linkedin.com/in/kevinz-lai/
"""
...
# TODO: Change the number of bars to sort
NUM_BARS = 20
...
def draw():
    ...
    for i in range(NUM_BARS):
        rectangles.append(
            canvas.create_rectangle(BORDER + UNIT_DIM * i, HEIGHT - BORDER,
                                    BORDER + UNIT_DIM * i + UNIT_DIM,
                                    HEIGHT - BORDER - UNIT_DIM / 2 * (bars[i]),
                                    fill="orange"))
...
def draw_buttons():
    ...
    f7 = Frame(tk, height=HEIGHT / 20, width=WIDTH / (NUM_SORTS + 1.5),
               bg='SeaGreen2')
    f7.pack_propagate(0)
    f7.pack(side='right')

    f6 = Frame(tk, height=HEIGHT / 20, width=WIDTH / (NUM_SORTS + 1.5),
               bg='salmon3')
    f6.pack_propagate(0)
    f6.pack(side='right')

    f5 = Frame(tk, height=HEIGHT / 20, width=WIDTH / (NUM_SORTS + 1.5),
               bg='SeaGreen2')
    f5.pack_propagate(0)
    f5.pack(side='right')

    f4 = Frame(tk, height=HEIGHT / 20, width=WIDTH / (NUM_SORTS + 1.5),
               bg='salmon3')
    f4.pack_propagate(0)
    f4.pack(side='right')

    f3 = Frame(tk, height=HEIGHT / 20, width=WIDTH / (NUM_SORTS + 1.5),
               bg='SeaGreen2')
    f3.pack_propagate(0)
    f3.pack(side='right')

    f2 = Frame(tk, height=HEIGHT / 20, width=WIDTH / (NUM_SORTS + 1.5),
               bg='salmon3')
    f2.pack_propagate(0)
    f2.pack(side='right')

    randomize_button = Button(f1, text='Randomize',
                              command=lambda: shuffle_one_by_one())
    randomize_button.pack(fill=BOTH, padx=20, pady=10)

    quick_sort_button = Button(f2, text='Quick Sort',
                               command=lambda: quick_sort(0, NUM_BARS - 1))
    ...
    bubble_sort_button = Button(f4, text='Bubble Sort',
                                command=lambda: bubble_sort())
    bubble_sort_button.pack(fill=BOTH, padx=20, pady=10)

    insertion_sort_button = Button(f5, text='Insertion Sort',
                                   command=lambda: insertion_sort())
    insertion_sort_button.pack(fill=BOTH, padx=20, pady=10)

    selection_sort_button = Button(f6, text='Selection Sort',
                                   command=lambda: selection_sort())
    selection_sort_button.pack(fill=BOTH, padx=20, pady=10)

    bogo_bogo_sort_button = Button(f7, text='Bogo-Bogo Sort',
                                   command=lambda: bogo_bogo_sort())
...
def switch_rectangles(index1, index2):
    ...
    rectangles[index1], rectangles[index2] = rectangles[index2], rectangles[
        index1]
